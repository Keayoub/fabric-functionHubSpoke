...
def resolve_caller(claims: dict) -> dict:
    ...
    logger.info(
        ...
        caller["type"], caller["display"], caller["oid"]
    )
    return caller
...
def get_sp_creds(kv: SecretClient) -> tuple[str, str, str]:
    ...
    """
    sp_id     = kv.get_secret(SP_CLIENT_ID_SECRET).value
    sp_secret = kv.get_secret(SP_CLIENT_SECRET_NAME).value
    sp_tid    = kv.get_secret(SP_TENANT_SECRET).value if SP_TENANT_SECRET else TENANT_ID

    logger.info("SP creds fetched from KV — sp_client_id=%s...", sp_id[:8])
...
def get_sp_token(
    ...
    scope = (
        target_scope if target_scope.endswith("/.default")
    ...
    if "error" in result:
        raise RuntimeError(
            f"MSAL error: {result['error']} — {result.get('error_description', '')}"
        )
...
def get_sp_token_handler(req: func.HttpRequest) -> func.HttpResponse:
    ...
    logger.info(
        ...
        caller["type"], caller["display"], target_scope
    )
    return func.HttpResponse(
        json.dumps(response),
        status_code=200,
